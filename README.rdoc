...
属性を定義する場合は, 以下のように attr_accessor ではなく column メソッドを利用すると便利です.
column メソッドを利用すると, 属性の型とデフォルト値を指定することができます.
...
データベースのテーブルに関連付いているモデルでも column メソッドを利用したい場合は,
EasyModel::Column モジュールを include します.
